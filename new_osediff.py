...
class OSEDiff_gen(torch.nn.Module):
    def __init__(self, args):
        ...
        self.unet, self.lora_unet_modules_encoder, self.lora_unet_modules_decoder, self.lora_unet_others = initialize_unet(
            self.args)
        if args.osediff_path is not None:
    ...
    def forward(self, c_t, batch=None, prompt="" ,args=None):
...
class OSEDiff_test(torch.nn.Module):
    ...
    # @perfcount
    @torch.no_grad()
    def forward(self, lq, pos_prompt, neg_prompt, use_cfg=False):

        pos_prompt_embeds = self.encode_prompt([pos_prompt])
        neg_prompt_embeds = self.encode_prompt([neg_prompt])
        ...
        # encoder_hidden_states = torch.cat([neg_prompt_embeds, pos_prompt_embeds])
        ## add tile function
        ...
        if h * w <= tile_size * tile_size:
            ...
            if use_cfg:
                pos_model_pred = self.unet(lq_latent, self.timesteps, encoder_hidden_states=pos_prompt_embeds).sample
                neg_model_pred = self.unet(lq_latent, self.timesteps, encoder_hidden_states=neg_prompt_embeds).sample
                model_pred = neg_model_pred + self.guidance_scale * (pos_model_pred - neg_model_pred)
            else:
                model_pred = self.unet(lq_latent, self.timesteps, encoder_hidden_states=pos_prompt_embeds).sample
            # neg_model_pred = self.unet(lq_latent, self.timesteps, encoder_hidden_states=neg_prompt_embeds).sample
            # model_pred = neg_model_pred + self.guidance_scale * (pos_model_pred - neg_model_pred)
            # model_out = self.unet(lq_latent, self.timesteps, encoder_hidden_states=encoder_hidden_states).sample
            # neg_model_pred, pos_model_pred = model_out.chunk(2)
            # model_pred = neg_model_pred + self.guidance_scale * (pos_model_pred - neg_model_pred)
        else:
            ...
            for row in range(grid_rows):
                ...
                for col in range(grid_cols):
                    ...
                    if len(input_list) == 1 or col == grid_cols-1:
                        ...
                        model_out = self.unet(input_list_t, self.timesteps, encoder_hidden_states=pos_prompt_embeds.to(self.weight_dtype),).sample
                        input_list = []
