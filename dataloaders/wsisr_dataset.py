    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            ...
            if self.deg_type == "realgan":
                ...
                lr_img = Image.open(self.lr_path + img_name).convert('RGB')
                output_t, img_t, sr_prompt, mag_prompt = self.degradation.random_augment_wsi_compress(np.asarray(gt_img) / 255.,np.asarray(lr_img) / 255.)
                output_t, img_t = output_t.squeeze(0), img_t.squeeze(0)
                self.sr_prompt = sr_prompt
                self.mag_prompt = mag_prompt
            ...
            if self.sr_prompt:
                ...
                example["pos_prompt"] = ""
            if self.mag_prompt:
                example["mag_prompt"] = self.mag_prompt
            example["null_prompt"] = ""
            example["output_pixel_values"] = output_t
            example["conditioning_pixel_values"] = img_t
...
class WSISRDataset_val_multi(torch.utils.data.Dataset):
    def __init__(self, split=None, args=None, mag="40x", scale = 4):
        super().__init__()
        self.args = args
        self.split = split
        self.mag = mag
        ...
        if split == 'test':
            ...
            self.gt_list.extend(sorted([str(x) for x in Path(self.gt_path).rglob('*.png')]))
            max_len = 4000
            if max_len is not None:   # 限制长度
                self.gt_list = self.gt_list[:max_len]
            print(f'=====> append {len(self.gt_list)} data.')
        ...
        if self.split == 'test':
            ...
            if self.deg_type == "realgan":
                ...
                if os.path.exists(png_path):
                    ...
                    raise FileNotFoundError(f"Neither {png_path} nor {jpg_path} exists.")
                output_t, img_t, sr_prompt = self.degradation.random_augment_multi(np.asarray(gt_img) / 255.,np.asarray(lr_img) / 255., mag=self.mag)
                output_t, img_t = output_t.squeeze(0), img_t.squeeze(0)
                self.sr_prompt = sr_prompt
            elif self.deg_type == "norm":
                output_t, img_t = self.degradation.random_augment_norm(np.asarray(gt_img) / 255.)
...
class WSISRDataset_test_multi(torch.utils.data.Dataset):
    def __init__(self, split=None, args=None):
        super().__init__()
        self.args = args
        self.split = split
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'test':
            ...
            output_t, img_t, sr_prompt = self.degradation.random_augment_multi(np.asarray(gt_img) / 255.,np.asarray(lr_img) / 255.)
            output_t, img_t = output_t.squeeze(0), img_t.squeeze(0)
            self.sr_prompt = sr_prompt
